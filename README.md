...
def check_ending():
    ...
    if s["친밀도"] >= 100 and s["기분"] >= 70 and s["건강"] >= 70:
        return f"🎉 최고의 친구 엔딩\n주인의 사랑덕분에 아가 다마는 사랑둥이 다마고치가 되었어요!"
    elif s["외로움"] >= 80:
        return f"🌧️ 조용한 이별 엔딩\n외로운 다마고치는 다른 주인을 찾아 떠나버렸어요"
    elif s["분노"] >= 70:
        return f"😡 분노 엔딩\n사랑 없이 혼나기만 한 다마고치는 떠나버렸어요"
    elif s["건강"] <= 30:
        return f"🤒 병약한 다마고치 엔딩\n다마고치는 병에 걸려 병원으로 떠났어요"
    elif s["친밀도"] < 20:
        return f"💔 친밀도 낮음 엔딩\n당신과의 어색함을 견디지 못한 다마는 당신을 떠났어요"
    elif s["기분"] <= 20:
        return f"😢 기분이 너무 안좋아서 떠나는 엔딩\n당신은 다마 달래기에 실패했습니다"
