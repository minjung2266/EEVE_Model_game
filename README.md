...
def check_ending():
    ...
    # 2. 외로움+분노 모두 높은 경우 (먼저 체크!)
    if s["외로움"] >= 80 and s["분노"] >= 80:
        return f"💥 폭발한 다마 엔딩 : 외롭고 화나서 다마고치는 폭발하고 말았어요!"

    # 3. 외로움만 높은 경우
    if s["외로움"] >= 80:
        return f"🌧️ 조용한 이별 엔딩 : 외로운 다마고치는 다른 주인을 찾아 떠나버렸어요"

    # 4. 분노만 높은 경우
    if s["분노"] >= 80:
        return f"😡 분노 엔딩 : 사랑 없이 혼나기만 한 다마고치는 떠나버렸어요"
    ...
    # 7. 기분이 안 좋은 경우
    if s["기분"] <= 20:
        return f"😢 기분이 너무 안좋아서 떠나버린 엔딩 : 당신은 다마 달래기에 실패했습니다"
